...
inline void rasterizer<VB, RT>::draw(size_t num_vertexes, size_t vertex_offset)
{
	...
	while (vertex_id < vertex_offset + num_vertexes)
	{
		...
		float2 bounding_box_begin{
			std::clamp(std::min(vertices[0].x, std::min(vertices[1].x, vertices[2].x)), 0.f, static_cast<float>(width) - 1.f),
			std::clamp(std::min(vertices[0].y, std::min(vertices[1].y, vertices[2].y)), 0.f, static_cast<float>(height) - 1.f)
		};
		float2 bounding_box_end{
			std::clamp(std::max(vertices[0].x, std::max(vertices[1].x, vertices[2].x)), 0.f, static_cast<float>(width) - 1.f),
			std::clamp(std::max(vertices[0].y, std::max(vertices[1].y, vertices[2].y)), 0.f, static_cast<float>(height) - 1.f)
		};
		...
		for (int x = static_cast<int>(bounding_box_begin.x); x <= static_cast<int>(bounding_box_end.x); x++)
		{
			for (int y = static_cast<int>(bounding_box_begin.y); y <= static_cast<int>(bounding_box_end.y); y++)
			{
				...
				if (edge0 >= 0.f && edge1 >= 0.f && edge2 >= 0.f)
				{
					...
					if (depth_test(z, x, y))
					{
						vertex interpolated_vertex = VB::interpolate_bary(vertices[0], vertices[1], vertices[2], u, v, w);
						auto pixel_shader_result = pixel_shader(interpolated_vertex, z);
						render_target->item(x, y) = cg::unsigned_color::from_color(pixel_shader_result);
